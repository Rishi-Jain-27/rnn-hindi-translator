...
It is *not* part of the active Phase C pipeline (which lives in `../src/nmt/`).
...
Phase C is a clean rebuild: SentencePiece subwords, IITB-scale data, and the full set of modeling / training /
inference / evaluation / analysis improvements (see `../CLAUDE.md`). SentencePiece changes the vocabulary, so
none of the Phase B checkpoints transfer — this notebook stays as the baseline that motivated the rebuild.
